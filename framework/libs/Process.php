...
class Process
{
	const PROCESS_NAME_LOG = ' php: swoole-bot'; //shell脚本管理标识
	...
	{
		$this->config = $config;
		$this->logger = new Log($config['path']);
	}

	public function start()
	{
		\Swoole\Process::daemon(true, true);
		isset($config['swoole']['workNum']) and $this->workNum = $this->config['swoole']['workNum'];
		//设置主进程
		$ppid = getmypid();
		...
		$this->registerSignal($this->workers);
	}
	...
	//监控子进程
	public function registerSignal(&$workers)
	{
		//监听exit信号
		...
		\Swoole\Process::signal(SIGCHLD, function($signo) use(&$workers) {
			while(true) {
				...
				if ($ret) {
					...
					echo "Worker Exit, kill_signal={$ret['signal']} PID=$pid \n";
					//杀掉子进程后，开启新的子进程
	...
	private function setExit()
	{
		...
		foreach($this->workers as $pid => $worker) {
			//平滑退出，用exit, 强制退出，使用kill
			\Swoole\Process::exit($pid);
			unset($this->workers[$pid]);
			$this->logger->log("主进程收到退出信号, [$pid], 子进程跟着退出\n");
			$this->logger->log("worker count: ".count($workers));
		}
		exit(0);
	}
